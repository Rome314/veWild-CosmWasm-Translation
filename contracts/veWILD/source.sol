...
contract VeToken is SafeOwnable, ReentrancyGuard {
    ...
    function lock(uint _amount, uint _newLockedUntil) external nonReentrant {
        ...
        require(
            _newLockedUntil >= lockedUntil[msg.sender],
            "VeToken: cannot reduce locked time"
        );
        require(
            !msg.sender.isContract(),
            "VeToken: contract calls not allowed"
        );
        ...
        if (_amount > 0) {
            ...
            totalLocked += _amount;
            IERC20(lockedToken).transferFrom(
                msg.sender,
                address(this),
                _amount
            );
    ...
    function requestWithdraw() external nonReentrant {
        ...
        require(
            ...
            "VeToken: cannot withdraw before unlock"
        );
    ...
    function withdraw() external nonReentrant {
        ...
        require(
            withdrawTime > 0 && withdrawTime <= block.timestamp,
            "VeToken: withdraw delay not over"
        );
    ...
    function pendingAccountReward(address _account) public view returns (uint) {
        ...
        uint rewardPerTokenDelta = pedingRewardPerToken -
            rewardSnapshot[_account];
    ...
    function _setDistributionPeriod(uint _blocks) internal {
        require(
            _blocks > 0,
            "VeToken: distribution period must be >= 100 blocks"
        );
        _accrue();
    ...
    function _updateRewardRate(
        ...
        uint blocksElapsed = Math.min(
            distributionPeriod,
            block.number - lastIncomeBlock
        );
        uint unvestedIncome = rewardRateStored *
            (distributionPeriod - blocksElapsed);

        rewardRateStored =
            (unvestedIncome + _addAmount) /
...
function _pendingRewardPerToken() internal view returns (uint) {
    ...
    function _checkReserves() internal view {
        ...
        require(
            reserveBalance >= totalLocked + unvestedIncome,
